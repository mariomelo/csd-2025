...
class GameEngine {
  ...
  handleEvent(event, data, currentGameState) {
    setInterval(() => {
      console.log(
        chalk.red(this.i18n.t("defaultTimer")) + currentGameState.defaultTimer
      );
      if (currentGameState.defaultTimer > 0) {
        currentGameState.defaultTimer--;
        console.log(`Timer: ${currentGameState.defaultTimer}`);
      }

      if (currentGameState.timer === 0 && currentGameState.status !== "LOST") {
        currentGameState.status = "LOST";
        console.log("Status changed to LOST");
        currentGameState.defaultTimer = 45;
      }
    }, 1000);
  }
