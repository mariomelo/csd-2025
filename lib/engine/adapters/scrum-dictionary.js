...
class ScrumDictionary extends DictionaryPort {
  ...
  getRandomWord(difficulty) {
    ...
    const filteredWords = this.words.filter(word => word.length <= maxLenght);
    const sizeFiltered = filteredWords.length;
    const randomIndex = Math.floor(Math.random() * sizeFiltered);
